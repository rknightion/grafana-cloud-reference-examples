...
def check_file_set(example_dir: Path, manifest: dict[str, Any], report: Report) -> None:
    ...
    for name in REQUIRED_DIRS_BY_DELIVERABLE.get(deliverable, ()):
        ...
        if not directory.is_dir():
            ...
            # An empty directory exists locally and does NOT exist in a fresh
            # checkout, because git cannot store one. Checking only is_dir()
            # passes on the author's machine and fails in CI.
            report.fail(
                where,
                f"{name}/ is empty, so it will not exist in a checkout - git cannot "
                f"store an empty directory. Put something in it or drop it.",
            )
